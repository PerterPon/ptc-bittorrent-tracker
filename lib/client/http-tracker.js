...
class HTTPTracker extends Tracker {
  ...
  async _request (requestUrl, params, cb) {
    ...
    let res
    let controller = abortTimeout(this.client.timeoutTime || common.REQUEST_TIMEOUT)
    try {
    ...
    if (warning) {
      ...
      err.warningType = 'responseWarning'
